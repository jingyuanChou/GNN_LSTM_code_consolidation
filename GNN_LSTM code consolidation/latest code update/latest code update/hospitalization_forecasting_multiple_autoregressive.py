...
def evaluateAR(data, X, Y, model, criterion, batch_size):
    model.eval()
    total_loss = 0
    n_samples = 0
    predict = None
...
def predict_next_timestamp_autoregressive(X, model, future_timestamps, mc_num):
    ...
    for num_iteration in range(mc_num):
        ...
        for i in range(future_timestamps):
            ...
            X = torch.cat((X[:,:,:,1:], temp), dim=3)
            output = torch.squeeze(output)
            X = torch.squeeze(X)
            list_of_prediction.append(output[-1,:])
            adj_mc_num.append(adj)
        next_4 = torch.stack(list_of_prediction, dim=1)
        mc_num_predictions.append(next_4.numpy())
...
def main(exp_name,exp_num, best_20_runs_train_adj_list, best_epoch_20_runs):
    ...
    if not args.pure_data_driven:
        if args.multivariate_TE_enhanced != args.BivariateTE:
            adj = load_TE_matrix(args.multivariate_TE_enhanced, args.BivariateTE)
    ...
    if args.Vector_Autoregression:
        VAR_baseline_result  = Vector_Autoregression(hopsitalization, args.seq_in_len,args.seq_out_len)
    ...
    if args.Plot_4_smaller_regions:
        with open("geographic_result_AR", "rb") as fp:  # Unpickling
            geo_result = pickle.load(fp)
            geo_result = np.mean(geo_result, axis=0)
            geo_result = np.mean(geo_result, axis=0)

        with open('pure_data_driven_result_AR', 'rb') as mtgnn_pure:
            mtgnn_result = pickle.load(mtgnn_pure)
            mtgnn_result = np.mean(mtgnn_result, axis=0)
            mtgnn_result = np.mean(mtgnn_result, axis=0)

        with open('BITE_enhanced_result_AR', 'rb') as Bi_result:
            Bi_result = pickle.load(Bi_result)
            Bi_result = np.mean(Bi_result, axis=0)
            Bi_result = np.mean(Bi_result, axis=0)
        with open('TE_predictions_results_AR', 'rb') as Te_result:
            Te_result = pickle.load(Te_result)
            Te_result = np.mean(Te_result, axis=0)
            Te_result = np.mean(Te_result, axis=0)
        for i in range(hopsitalization.shape[1]):
            x_range = range(hopsitalization.shape[0] - 10 + 1, hopsitalization.shape[0] + 1)
            plt.plot(x_range, hopsitalization[-10:, i], marker='>', markersize=2.0, label='ground truth')
            prev_6_timestamps = hopsitalization[-10:-4, i]
            next_4_timestamps_geo = geo_result[i]
            next_4_timestamps_mtgnn = mtgnn_result[i]
            next_4_timestamps_Biresult = Bi_result[i]
            ...
            x_value_Biresult = np.concatenate((prev_6_timestamps, next_4_timestamps_Biresult), axis=None)
            ...
            plt.plot(x_range[5:], x_value_geo[5:], linestyle='--', marker='o',
                     markersize=2.0, label='geo-based')

            plt.plot(x_range[5:], x_value_mtgnn[5:], linestyle='--', marker='>',
                     markersize=2.0, label='mtgnn')

            plt.plot(x_range[5:], x_value_Biresult[5:], linestyle='--', marker='<',
                     markersize=2.0, label='Bivaraite TE')

            plt.plot(x_range[5:], x_value_TEresult[5:], linestyle='--', marker='<',
                     markersize=2.0, label='Multivariate TE')
    ...
    for iter in range(exp_num):
        ...
        Data = DataLoaderS_hosp(available_data, 0.8, 0.2, args.device,args.train_horizon, args.horizon, args.seq_in_len,
        ...
        best_model_path = 'best_model_AR.pth'
        for epoch in range(1, args.epochs + 1):
            train_loss, adj_list = train(Data, Data.train[0], Data.train[1], model, criterion, optim, args.batch_size)
            val_loss, adj_list_val = evaluateAR(Data, Data.valid[0], Data.valid[1], model, criterion,
                                              args.batch_size)
        ...
        next_4_pred = predict_next_timestamp_autoregressive(Data.predict_data,best_model, args.horizon, 50)
    ...
    if not args.buildA_true:
        if exp_name == 'geo': # geo matrix based, without graph learning
            with open("geo_static_performance_result_subgraph_size_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha,exp_num), "wb") as geo:  # Pickling
                pickle.dump(final_20_run_results, geo)
            with open("geo_static_final_pred_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size,args.tanhalpha,exp_num), "wb") as geo_result:  # Pickling
                pickle.dump(list_of_next_4_since_current_time, geo_result)
            with open("geo_static_adj_matrix_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size,args.tanhalpha,exp_num), "wb") as geo_result_adj:  # Pickling
            ...
            with open("MTE_static_performance_result_subgraph_size_{}_and_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha,exp_num), "wb") as MTE_static:  # Pickling
                pickle.dump(final_20_run_results, MTE_static)
            with open("MTE_static_final_pred_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size,args.tanhalpha,exp_num), "wb") as MTE_static_result:  # Pickling
                pickle.dump(list_of_next_4_since_current_time, MTE_static_result)
            with open("MTE_static_adj_matrix_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size,args.tanhalpha,exp_num), "wb") as MTE_static_adj:  # Pickling
                pickle.dump(best_20_runs_train_adj_list, MTE_static_adj)

    else:
        if args.multivariate_TE_enhanced: # MTE enhanced, with graph learning.
            with open("MTE_adaptive_performance_result_subgraph_size_{}_and_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha, exp_num), "wb") as DD:  # Pickling
                pickle.dump(final_20_run_results, DD)
            with open("MTE_adaptive_final_pred_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha, exp_num), "wb") as DD_result:  # Pickling
                pickle.dump(list_of_next_4_since_current_time, DD_result)
            with open("MTE_adaptive_adj_matrix_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha, exp_num), "wb") as MTE_result_adj:  # Pickling
                pickle.dump(best_20_runs_train_adj_list, MTE_result_adj)
        else: # Pure MTGNN, with graph learning
            with open("MTGNN_adaptive_performance_result_subgraph_size_{}_and_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha,exp_num), "wb") as PUREDATA:  # Pickling
                pickle.dump(final_20_run_results, PUREDATA)
            with open("MTGNN_adaptive_final_pred_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha, exp_num), "wb") as pure_data_result:  # Pickling
                pickle.dump(list_of_next_4_since_current_time, pure_data_result)
            with open("MTGNN_adaptive_adj_matrix_subgraph_{}_and_alpha_{}_expnum_{}_AR".format(args.subgraph_size, args.tanhalpha, exp_num), "wb") as pure_data_result_adj:  # Pickling
