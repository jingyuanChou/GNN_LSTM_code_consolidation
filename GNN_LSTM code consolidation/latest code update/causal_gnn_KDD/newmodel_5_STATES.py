...
if __name__ == '__main__':
    ...
    '''
    # plots
    reader = open('IDENTITY_5_states_weekly_recrusive_predictions_vs_labels_4_week_ahead_regularized_12_weeks_mean_variance.json')
    one_week = json.load(reader)
    # idx_list = [0,5,10,15,20]
    LSTM_only_included = deepcopy(one_week)
    MAEP = dict()
    ...
    reader = open(
        '5_states_weekly_recrusive_predictions_vs_labels_4_week_ahead_regularized_12_weeks_mean_variance.json')
    one_week = json.load(reader)
    idx_list = [1, 5, 10, 16, 22]
    for can in candidate_states:
        all_values_single_state = hopsitalization[can].values
        for index, time in enumerate(timestamps):
            starting_point = num_prev_time_steps + time - 4
