...
class LSTM(torch.nn.Module):
    ...
    def _set_cell_state(self, X, C):
        if C is None:
            C = torch.zeros(X.shape[0], X.shape[1], self.out_channels)
        return C
...
class VanillaRNN(torch.nn.Module):
    ...
    def forward(self, X: torch.FloatTensor, edge_index: torch.LongTensor, edge_weight: torch.FloatTensor=None, H: torch.FloatTensor=None, C: torch.FloatTensor=None):
        ...
        # Vanilla RNN doesn't have cell state
...
class RNN(torch.nn.Module):
    ...
    """
    def __init__(self, node_features=1, output=4, dim=32, module=GraphLinear, rnn = LSTM, gnn=WeightedSAGEConv, gnn_2=WeightedSAGEConv, rnn_depth=1, name="RNN", skip_connection=True):
        ...
        self.lin1 = torch.nn.Linear(2 * dim, output)
